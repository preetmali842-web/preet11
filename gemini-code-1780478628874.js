...
class AppStore {
    constructor() {
        ...
        this.defaultState = {
            theme: 'light', // Updated default to Light based on UI preference
            habits: [
...
// --- Utilities ($1000 Feature: Export to PNG via html2canvas) ---
function exportDashboardAsPNG() {
    ...
    // We force html2canvas to render the background based on the current theme
    const themeBg = getComputedStyle(document.body).getPropertyValue('--bg-base');

    html2canvas(targetElement, {
        backgroundColor: themeBg,
        scale: 2, // High resolution for mobile/desktop
        useCORS: true,
        logging: false
    }).then(canvas => {
        ...
        a.href = url;
        a.download = `AIR1_Stats_${store.getTodayStr()}.png`;
        a.click();
        showToast("Dashboard successfully saved as PNG!");
    }).catch(err => {
